...
Scenario: Get the histories of a user
  ...
  @US_055 @1
  Scenario: Error message when the userId on the history doesn't exist
    Given I want to retrieve a error message when the user id on the history is "invalid"
    And I am targeting endpoint for "get_histories"
    When I send a GET request to get the histories
  ...
  @US_055 @2
  Scenario: Get the histories of a user
    Given I want to retrieve a error message when the user id on the history is "valid"
    And I am targeting endpoint for "get_histories"
    When I send a GET request to get the histories
    Then The status code should be "200"
    And The number of registered histories should be the same of retrieved histories
